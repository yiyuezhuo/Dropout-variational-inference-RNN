...
def apply_dropout_batch(W, prob, batch_size):
    '''
    Expect input X: 
        (batch, 1, in_feature)
    Expect process:
    ...
    mask = (torch.rand(mask_size) < prob).type(W.dtype)
    masked_W = mask * W
...
class BayesLSTM(MyLSTM):
    ...
    def forward(self, x, hx=None):
        ...
        T = lambda W: apply_dropout_batch(W, self.dropout_prob, batch_size) 
...
class RegressionLSTMBayes(nn.Module):
    ...
    def __init__(self, input_features, hidden_dims, dropout_prob = 0.5):
        ...
        self.rnn = BayesLSTM(dropout_prob, lstm_bb.weight_ih_l0.clone(), lstm_bb.weight_hh_l0.clone(), lstm_bb.bias_ih_l0.clone(), lstm_bb.bias_hh_l0.clone())
